...
import { addCurrentSelected } from "../../features/articles/articlesSlice";
...
const BlogDetailsDynamic = () => {
  ...
  const { currentSelected } = useSelector((state) => state.articles) || {};
  const { isLoading, data, error,isError, isSuccess } = useGetArticleByIdQuery({id})

  useEffect(() => {

    if (!id) <h1>Loading...</h1>;
    else if(isSuccess){
      dispatch(addCurrentSelected(data.data));
              ...
              <div className="main_blog_post_content">
                <div className="mbp_thumb_post">
                  <div className="blog_sp_tag">
                    <a href="#">{data.data?.topic.name}</a>
                  </div>
                  <h3 className="blog_sp_title">{data.data?.title}</h3>
